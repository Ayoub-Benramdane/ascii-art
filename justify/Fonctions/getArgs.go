...
func GetArgs(args []string, clo, letters_to_be_colored, output *[]string, align, str, banner *string) { // clo slice of color
	...
	for i := 0; i < len(args) && strings.Index(args[i], "--") == 0; i++ {
		index++
		*output, color, *align, clout = GetOption(args[i], color, *align, *output, &clout)
...
func GetOption(arg, color, align string, output []string, clout *bool) ([]string, string, string, bool) {
